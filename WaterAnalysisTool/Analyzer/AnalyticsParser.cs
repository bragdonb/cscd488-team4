        ...
        private List<Element> fillElementList()
        {
            ...
            for (int x = 0; this.dataws.Cells[this.row, this.col].Value != null; x++)
            {
                ...
                for (int y = 0; this.dataws.Cells[this.row, this.col].Value != null; y++)
                {
                    ...
                    analytes.Add(new Element(this.elementNames[x], "", Double.Parse(this.dataws.Cells[this.row, this.col].Value.ToString()), this.row, this.col)); // using stddev and rsd to store row and col for graph generation
                    this.row++;
                    if (firstRun)
                        colLength++;
                }
